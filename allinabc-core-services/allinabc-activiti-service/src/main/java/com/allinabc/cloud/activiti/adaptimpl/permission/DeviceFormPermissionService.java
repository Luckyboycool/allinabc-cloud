...
@Service
@Slf4j
public class DeviceFormPermissionService implements FormPermissionAdapt {
    ...
    public void saveStartFormPerms(String formType, String businessId, String accountType, String userId, Map<String, Object> variables,String taskDefKey,String procDefKey) {
        log.info("Device启动流程保存权限");
//        1.申请人及申请人所在账号组内的所有人员
//        2.历史审批人
//        3.查看所有申请单功能项人员   Device表单的“Foundry 区域”取对应的CE、PIE、DS、Mask、Mask（DS）群组  合并到一张表
        ...
        if (accountType.equals(ProcessNodeConstant.ACCOUNT_TYPE_EMPLOYEE)) {
            ...
            log.info("存入改版的权限");
            FormPermission reversionFormPermision = getFormPermision(userId, formType, businessId, ProcessNodeConstant.ACCOUNT_TYPE_EMPLOYEE, userId, ProcessNodeConstant.REVISION);
            formPermissionMapper.insert(reversionFormPermision);
            log.info("存入改版的权限结束");
            ...
            if(groupIds!=null && groupIds.size()>0){
                ...
                groupIds.forEach(group->{
                    ...
                    FormPermission var2 = getFormPermision(userId, formType, businessId, ProcessNodeConstant.ACCOUNT_TYPE_GROUP, group, ProcessNodeConstant.REVISION);
           ...
           //        Device表单的“Foundry 区域”取对应的CE、PIE、DS、Mask、Mask（DS）群组
            ...
            childAndParentById.forEach(s -> {
                FormPermission read = getFormPermision(userId, formType, businessId, ProcessNodeConstant.ACCOUNT_TYPE_ACCOUNT, s.getId(), ProcessNodeConstant.READ);
                log.info("新增客户账户查看权限");
                formPermissionMapper.insert(read);
                FormPermission revision = getFormPermision(userId, formType, businessId, ProcessNodeConstant.ACCOUNT_TYPE_ACCOUNT, s.getId(), ProcessNodeConstant.REVISION);
                log.info("新增客户账户改版权限");
                formPermissionMapper.insert(revision);
            });
        } else {
            log.error("账户类型为未知类型");
        }
        ...
        groupNameList.stream().forEach(j->{
            if(j.getIsFab().equals("N")){
                ...
                log.info("Device表单的“Foundry 区域”取对应的CE、PIE、DS、Mask、Mask（DS）群组=" + JSON.toJSONString(var1));
                formPermissionMapper.insert(var1);
            }else {
    ...
    public void saveExecuteFormPerms(String formType, String businessId, String accountType, String userId, Map<String, Object> variables,String taskDefKey,String procDefKey,String startUser) {
        ...
        if(taskDefKey.equals(ProcessNodeConstant.DEVICE_CREATE)){
            log.info("当前节点为："+ProcessNodeConstant.DEVICE_CREATE+" 需要删除之前的group群组的权限，然后再插入现在群组的权限");
           ...
           if(accountType.equals(ProcessNodeConstant.ACCOUNT_TYPE_EMPLOYEE)) {
            ...
            groupFuncList.stream().forEach(gruop->{
                ...
                log.info("新增新group参数="+JSON.toJSONString(var2));
                formPermissionMapper.insert(var2);
                log.info("插入新group成功");
            });
        }
