...
/**
 * @Description  根据所选的fab查询对应的群组 type=8
 * @Author wangtaifeng
 * @Date 2021/3/22 19:05
 **/
@Service("node_auditor_type_8")
@Slf4j
public class FabGroupAuditorService implements NodeAuditorAdapt {
    ...
    public void setAuditor(String auditorKey, String nodeId, String nodeKey, String processDefinitionKey, String processInsId, String starter, Map<String, Object> variables) {
        log.info("根据所选的fab查询对应的群组;类型=【8】");
        String fab = (String)variables.get("fab");
        String formType = (String)variables.get("formType");
        if(StringUtils.isEmpty(fab)){
            throw new RuntimeException("fab为空");
        }
        if(StringUtils.isEmpty(formType)){
            throw new RuntimeException("formType为空");
        }
        String groupName =  nodeAuditorMapper.selectGroupNameByFabCode(fab,formType,nodeKey);
        if(StringUtils.isEmpty(groupName)){
            throw new RuntimeException("根据所选的fab未查询到对应的groupName");
        }
        List<String> groupUserIds = nodeAuditorMapper.selectGroupAuditorByGroupName(groupName);
        groupUserIds = groupUserIds.stream().filter(s-> !StringUtils.isEmpty(s)).collect(Collectors.toList());

        log.info("下一节点的审批人为" + JSON.toJSONString(groupUserIds));
        ...
        if (!CollUtil.isEmpty(groupUserIds)) {
            ...
            groupUserIds.stream().distinct().forEach(id -> {
    ...
    public void setAuditor(String auditorKey, String nodeId, String nodeKey, String processDefinitionKey, String processInsId, String starter, NodeJumpParam nodeJumpParam) {
        this.setAuditor(auditorKey,nodeId,nodeKey,processDefinitionKey,processInsId,starter,nodeJumpParam.getVariables());
    }
