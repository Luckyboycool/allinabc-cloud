package com.allinabc.cloud.gateway.filter;
...
public class AuthFilter implements GlobalFilter, Ordered {
    ...
    // 排除过滤的 uri 地址
    // swagger排除自行添加
    private static final List<String> whiteList = Arrays.asList("/auth/login","/user/register", "/v2/api-docs","/v2/api-docs-ext");
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        log.info("url:{}", url);
        // 跳过不需要验证的路径
        if (whiteList.contains(url) || url.endsWith(whiteList.get(2)) || url.endsWith(whiteList.get(3)) ) {
            return chain.filter(exchange);
        }
        // 校验是否为开发模式，开发模式可以不校验权限
        String devKey = exchange.getRequest().getHeaders().getFirst(Constants.DEV_MODEL_KEY);
        ...
        ServerHttpRequest mutableReq = exchange.getRequest().mutate().header(Constants.CURRENT_ID, userId)
                .header(Constants.CURRENT_USERNAME, jo.getString("userName")).build();
    ...
    private Mono<Void> setUnauthorizedResponse(ServerWebExchange exchange, String msg) {
        ...
        byte[] response = null;
        response = JSON.toJSONString(Result.failed(ApiResultCode.VALIDATE_AUTH_FAILED, msg)).getBytes(StandardCharsets.UTF_8);
